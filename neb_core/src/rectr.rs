...
impl RoundedRect {
    ...
    pub fn set_center(&mut self, rect: Rect) {
    ...
    fn path_elements(&self, tolerance: f64) -> RoundedRectPathIter {
        ...
        let arcs = [
            build_arc_iter(
                2,
                Point {
                    x: self.center_rect.x0 + radii.top_left,
                    y: self.center_rect.y0 + radii.top_left,
                },
                Vec2 {
            ...
            build_arc_iter(
                3,
                Point {
                    x: self.center_rect.x1 - radii.top_right,
                    y: self.center_rect.y0 + radii.top_right,
                },
                Vec2 {
                    x: radii.top_right - (self.center_rect.x1 - self.rect.x1).abs(),
            ...
            build_arc_iter(
                0,
                Point {
                    x: self.center_rect.x1 - radii.bottom_right,
                    y: self.center_rect.y1 - radii.bottom_right,
                },
                Vec2 {
                    x: radii.bottom_right - (self.center_rect.x1 - self.rect.x1).abs(),
            ...
            build_arc_iter(
                1,
                Point {
                    x: self.center_rect.x0 + radii.bottom_left,
                    y: self.center_rect.y1 - radii.bottom_left,
                },
                Vec2 {
                    x: radii.bottom_left - (self.center_rect.x0 - self.rect.x0).abs(),
                    y: radii.bottom_left - (self.center_rect.y1 - self.rect.y1).abs(),
                },
            ),
        ];
    ...
    fn next(&mut self) -> Option<PathEl> {
        ...
        match self.ix {
            1 => Some(PathEl::MoveTo(Point::new(
                self.rect.x0,
                self.rect.y0 + self.radii.top_left - (self.center_rect.y0 - self.rect.y0).abs(),
            ))),
            2 => Some(PathEl::LineTo(Point::new(
                self.rect.x1 - self.radii.top_right + (self.center_rect.x1 - self.rect.x1).abs(),
                self.rect.y0,
            ))),
            3 => Some(PathEl::LineTo(Point::new(
                self.rect.x1,
                self.rect.y1 - self.radii.bottom_right + (self.center_rect.y1 - self.rect.y1).abs(),
            ))),
            4 => Some(PathEl::LineTo(Point::new(
                self.rect.x0 + self.radii.bottom_left - (self.center_rect.x0 - self.rect.x0).abs(),
                self.rect.y1,
            ))),
